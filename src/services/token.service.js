...
const generateAuthTokens = async (user) => {
  let expirationDate=Math.floor(Date.now()/1000)+config.jwt.accessExpirationMinutes*60;
  let accessToken=generateToken(user._id,expirationDate,tokenTypes.ACCESS)
  return{
    access:{
      token:accessToken,
