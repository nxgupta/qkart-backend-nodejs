...
const addProductToCart = async (user, productId, quantity) => {
  let productToAdd = await productService.getProductById(productId);
  if (!productToAdd) {
    throw new ApiError(
  ...
  if (!cart) {
    ...
    let alreadyInCart = cart.cartItems.find(
    ...
    if (alreadyInCart) {
      ...
      cart.cartItems.push({product:productToAdd,quantity:quantity})
      await cart.save()
      return cart;
    }
  }
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  let cartItem = cart.cartItems.findIndex(
    (item) => item.product._id == productId
  );
  if(cartItem==-1) {
    throw new ApiError(
    ...
    cart.cartItems[cartItem].quantity=quantity
    await cart.save()
    return cart
  }
};
...
const deleteProductFromCart = async (user, productId) => {
  ...
  if(cartItem==-1) {
    ...
    cart.cartItems.splice(cartItem,1)
    await cart.save()
    return cart
  }
};
...
const checkout = async (user) => {
  ...
  let userData=await getCartByUser(user);
  let total=0;
